...
server.get("/invite", (request: Request, response: Response) => {
	response.redirect("https://discord.gg/UNXBMwMxHX");
})
...
export function websocketCon(client: WebSocket, request: IncomingMessage) {
	...
	client.send(JSON.stringify(<Chat.Send>{
		error: false,
		username: "C-LOG",
		message: `Welcome to atrium admin global chat, "C-LOG" means Client Log (log message will only be shown to you) "G-LOG" means Global Log (log message will be shown to everyone). There are currently ${Array.from(wss.clients).length} user (s) connected.`,
		tagColour: [8, 255, 8],
		clientOnly: true
	}));
