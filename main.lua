...
do
    ...
    if (F_A) then
        ...
        return Notify(nil, "Loaded", "atrium admin is already loaded... use 'killscript' to kill", nil),
...
local WriteConfig = function(Destroy)
    ...
    if (isfolder("atrium-admin") and Destroy) then
        delfolder("atrium-admin");
        writefile("atrium-admin/config.json", JSON);
        writefile("atrium/admin/pluings/plugin-conf.json", PluginJSON);
    else
        makefolder("atrium-admin");
        makefolder("atrium-admin/plugins");
        makefolder("atrium-admin/chatlogs");
        writefile("atrium-admin/config.json", JSON);
        writefile("atrium-admin/plugins/plugin-conf.json", PluginJSON);
    end
end

local GetConfig = function()
    if (isfolder("atrium-admin") and isfile("atrium-admin/config.json")) then
        return JSONDecode(Services.HttpService, readfile("atrium-admin/config.json"));
    else
        WriteConfig();
        return JSONDecode(Services.HttpService, readfile("atrium-admin/config.json"));
    end
...
local GetPluginConfig = function()
    if (isfolder("atrium-admin") and isfolder("atrium-admin/plugins") and isfile("atrium-admin/plugins/plugin-conf.json")) then
        local JSON = JSONDecode(Services.HttpService, readfile("atrium-admin/plugins/plugin-conf.json"));
        return JSON
    else
        WriteConfig();
        return JSONDecode(Services.HttpService, readfile("atrium-admin/plugins/plugin-conf.json"));
    end
end
...
local SetPluginConfig = function(conf)
    ...
    writefile("atrium-admin/plugins/plugin-conf.json", JSONEncode(Services.HttpService, NewConfig));
end

local SetConfig = function(conf)
    if (not isfolder("atrium-admin") and isfile("atrium-admin/config.json")) then
        WriteConfig();
    end
    ...
    writefile("atrium-admin/config.json", JSONEncode(Services.HttpService, NewConfig));
end
...
local WriteThemeConfig = function(Conf)
    if (IsSupportedExploit and isfolder("atrium-admin")) then
        ...
        writefile("atrium-admin/Theme.json", Data);
    end
end

GetThemeConfig = function()
    if (IsSupportedExploit and isfolder("atrium-admin")) then
        if (isfile("atrium-admin/Theme.json")) then
            local Success, Data = pcall(JSONDecode, Services.HttpService, readfile("atrium-admin/Theme.json"));
...
AddCommand("commandcount", {"cc"}, "shows you how many commands there is in atrium admin", {}, function(Caller)
...
AddCommand("dupetools2", {"rejoindupe", "dupe2"}, "sometimes a faster dupetools", {1,"1"}, function(Caller, Args)
    ...
    writefile("atrium-admin/tooldupe.txt", tostring(Amount - 1));
    writefile("atrium-admin/tooldupe.lua", format([[
        local OldPos = CFrame.new(%s);
        local DupeAmount = tonumber(readfile("atrium-admin/tooldupe.txt"));
        local game = game
        local GetService = game.GetService
        ...
        if (DupeAmount <= 1) then
            ...
            writefile("atrium-admin/tooldupe.txt", tostring(DupeAmount - 1));
            local queue_on_teleport = syn and syn.queue_on_teleport or queue_on_teleport
            queue_on_teleport(readfile("atrium-admin/tooldupe.lua"));
...
AddCommand("joinserver", {"discord"}, "joins the atrium admin discord server", {}, function()
    ...
    if (Request({
        ...
        return "joined atrium admin discord server"
    else
...
AddCommand("commands", {"cmds"}, "shows you all the commands listed in atrium admin", {}, function()
...
AddCommand("config", {"conf"}, "shows atrium admin config", {}, function(Caller, Args, CEnv)
...
AddConnection(CConnect(HttpLogs.Save.MouseButton1Click, function()
    local Children = GetChildren(HttpLogs.Frame.List);
    local Logs =  format("atrium Admin HttpLogs for %s\n\n", os.date());
    for i = 1, #Children do
        local v = Children[i]
        if (not IsA(v, "UIListLayout")) then
            Logs = format("%s%s\n", Logs, v.Text);
    ...
    if (not isfolder("atrium-admin/httplogs")) then
        makefolder("atrium-admin/httplogs");
    end
    writefile(format("atrium-admin/httplogs/HttpLogs for %s", gsub(tostring(os.date("%X")), ":", "-")) .. ".txt", gsub(Logs, "%b<>", ""));
...
AddConnection(CConnect(Console.Save.MouseButton1Click, function()
    ...
    local TimeSaved = gsub(tostring(os.date("%x")), "/", "-") .. " " .. gsub(tostring(os.date("%X")), ":", "-");
    local Children = GetChildren(Console.Frame.List);
    local String =  format("atrium Admin logs %s\nGame: %s - %d\n\n", TimeSaved, GameName, game.PlaceId);
    ...
    writefile("atrium-admin/logs.txt", String);
...
do
    ...
    if (PluginConf.PluginsEnabled) then
        ...
        if (IsSupportedExploit) then
            if (not isfolder("atrium-admin") and not isfolder("atrium-admin/plugins") and not isfolder("atrium-admin/plugin-conf.json") or not isfolder("atrium-admin/chatlogs")) then
                WriteConfig();
            end
        end
        ...
        AddCommand("refreshplugins", {"rfp", "refreshp", "reloadp"}, "Loads all new plugins.", {}, function()
            ...
            Plugins = map(filter(listfiles("atrium-admin/plugins"), function(i, v)
                return lower(split(v, ".")[#split(v, ".")]) == "lua"
            end), function(i, v)
...
_L.LatestCommit = JSONDecode(Services.HttpService, game.HttpGetAsync(game, "https://raw.githubusercontent.com/AtriumAdmin/Atrium/main/branch.lua"))[1]
wait(1);
